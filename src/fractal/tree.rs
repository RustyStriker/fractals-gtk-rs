...
pub fn tree_fractal(data : FractalInitData) -> Option<JoinHandle<()>>{
    // Clear the mutex 
    {
    ...
    let handle = thread::spawn(move ||{
        tree_fractal_helper(data);
    });
    Some(handle)
}

fn tree_fractal_helper(data : TreeData){
    {
        let color = (1.0 - (data.from.1 / data.size.1), data.branches as f64 / 255.0, 1.0 - (data.from.0 / data.size.0));
        let line = Line::new(data.from, data.to, color,2.0);
    ...
    if data.branches > 0{
        ...
        let to_1 = in_angle(len, -3.14 * 0.1 + angle, data.to);
        let to_2 = in_angle(len, 3.14 * 0.1 + angle, data.to);
        let to_4 = in_angle(len, -3.14 * 0.3 + angle, data.to);
        let to_5 = in_angle(len, 3.14 * 0.3 + angle, data.to);

        tree_fractal_helper(TreeData{
